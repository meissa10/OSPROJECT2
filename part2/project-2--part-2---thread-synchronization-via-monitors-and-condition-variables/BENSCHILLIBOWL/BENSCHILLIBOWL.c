...
BENSCHILLIBOWL* OpenRestaurant(int max_size, int expected_num_orders) {
  ...
  new_rest = (BENSCHILLIBOWL *) shmat(ShmID, NULL, 0);
  
  new_rest->orders = NULL;
  new_rest->current_size = 0;
  new_rest->max_size = max_size;
  new_rest->next_order_number = 0;
  new_rest->orders_handled = 0;
  new_rest->expected_num_orders = expected_num_orders;
  new_rest->mutex = (pthread_mutex_t) PTHREAD_MUTEX_INITIALIZER;
  new_rest->can_add_orders = (pthread_cond_t) PTHREAD_COND_INITIALIZER;
  new_rest->can_get_orders = (pthread_cond_t) PTHREAD_COND_INITIALIZER;  
